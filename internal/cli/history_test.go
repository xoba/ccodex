...
func (h *fakeHistory) RecordSamples(_ context.Context, samples []history.Sample, force bool) error {
	if h.writeErr != nil {
		return h.writeErr
	}
...
func TestStatusSavesSamplesUnlessHistoryIsOff(t *testing.T) {
	for _, test := range []struct {
		...
		if got.Account != "3001f754dd60" || got.Plan != "pro" || got.LimitID != "codex" || got.Dimension != "primary" ||
			got.UsedPercent != 40 || got.RemainingPercent != 60 || *got.ResetCredits != 2 || store.forced[0] {
			t.Fatalf("unexpected sample: %+v forced=%v", got, store.forced)
		}
	}
}
...
func TestWatchPrunesOldSamplesOncePerDay(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		recorder.samples(context.Background(), autoSnapshot(40, 2), false)
	}
...
func TestManualResetIsRecordedButNeverBlockedByHistory(t *testing.T) {
	for _, writeErr := range []error{nil, errors.New("disk full")} {
		...
		if len(store.samples) != 1 || !store.forced[0] || store.samples[0].UsedPercent != 0 {
			t.Fatalf("reading after the reset was not kept: %+v forced=%v", store.samples, store.forced)
		}
	}
}
...
func historyFixture() *fakeHistory {
	at := time.Date(2026, 9, 17, 14, 3, 12, 0, time.UTC)
	threshold, credits, mins := 5.0, int64(2), int64(300)
	...
	return &fakeHistory{
		...
		samples: []history.Sample{{
			Time: at, Account: "3001f754dd60", Plan: "pro", LimitID: "codex", Dimension: "primary", WindowMins: &mins,
			UsedPercent: 96.5, RemainingPercent: 3.5, ResetsAt: &resetsAt, ResetCredits: &credits,
		}, {
			Time: at.Add(time.Minute), LimitID: "codex", Dimension: "individual", UsedPercent: 70, RemainingPercent: 30,
		}},
	}
}
...
func TestHistoryCommandFormats(t *testing.T) {
	store := historyFixture()
	local := func(at time.Time) string { return at.Local().Format(historyTimeLayout) }
	at := store.events[0].Time

	text := runHistory(t, store, "history", "--since", "all")
	for _, want := range []string{
		"TIME", "REQUEST", local(at) + "  auto", "…333344445555", "codex primary 3.5% left; threshold 5%; 2 resets available",
		"outcome    reset", "daily limit reached", "=cmd|' /C calc'!A0 [31m next",
	} {
		if !strings.Contains(text, want) {
			t.Errorf("reset table lacks %q:\n%s", want, text)
		}
	}
	if strings.ContainsAny(text, "\x1b") || runHistory(t, store, "history", "resets", "--since", "all") != text {
		t.Errorf("unexpected reset table:\n%s", text)
	}

	wantCSV := `time,mode,event,outcome,key,account,detail,reason,version
2026-09-17T14:03:12Z,auto,requested,,auto-3001f754dd60-1111-2222-333344445555,3001f754dd60,,"{""trigger"":""lowQuota"",""threshold"":5,""low"":[{""limitId"":""codex"",""dimension"":""primary"",""remainingPercent"":3.5}],""resetCredits"":2,""maxResetsPerDay"":1}",1.2.3
2026-09-17T14:03:14Z,auto,outcome,reset,auto-3001f754dd60-1111-2222-333344445555,,,,
2026-09-17T15:03:12Z,auto,skipped,,,,dailyLimitReached,,
2026-09-17T16:03:12Z,manual,error,,k,,'=cmd|' /C calc'!A0 [31m next,,
`
	if got := runHistory(t, store, "history", "--since", "all", "--csv"); got != wantCSV {
		t.Errorf("reset CSV:\n got %s\nwant %s", got, wantCSV)
	}

	wantUsage := `time,account,plan,limit_id,dimension,window_mins,used_percent,remaining_percent,resets_at,reset_credits
2026-09-17T14:03:12Z,3001f754dd60,pro,codex,primary,300,96.5,3.5,2026-09-17T16:03:12Z,2
2026-09-17T14:04:12Z,,,codex,individual,,70,30,,
`
	if got := runHistory(t, store, "history", "usage", "--since", "all", "--csv"); got != wantUsage {
		t.Errorf("usage CSV:\n got %s\nwant %s", got, wantUsage)
	}
	wantJSON := `{"time":"2026-09-17T14:03:12Z","account":"3001f754dd60","plan":"pro","limitId":"codex","dimension":"primary","windowMins":300,"usedPercent":96.5,"remainingPercent":3.5,"resetsAt":"2026-09-17T16:03:12Z","resetCredits":2}
{"time":"2026-09-17T14:04:12Z","account":"","limitId":"codex","dimension":"individual","usedPercent":70,"remainingPercent":30}
`
	if got := runHistory(t, store, "history", "usage", "--since", "all", "--json"); got != wantJSON {
		t.Errorf("usage JSON:\n got %s\nwant %s", got, wantJSON)
	}
	usage := runHistory(t, store, "history", "usage", "--since", "all")
	for _, want := range []string{"RESETS AVAILABLE", local(at) + "  3001f754dd60  codex  primary (5h)  96.5%  3.5%", "individual"} {
		if !strings.Contains(usage, want) {
			t.Errorf("usage table lacks %q:\n%s", want, usage)
		}
	}
	if got := runHistory(t, store, "history", "resets", "--json", "--since", "all"); strings.Count(got, "\n") != 4 || !strings.HasPrefix(got, `{"time":"2026-09-17T14:03:12Z","key":"auto-`) {
		t.Errorf("reset JSON:\n%s", got)
	}

	// Everything in the fixture is older than the default windows.
	if got := runHistory(t, &fakeHistory{}, "history"); !strings.HasPrefix(got, "No reset events recorded since ") {
		t.Errorf("empty reset history: %q", got)
	}
	if got := runHistory(t, &fakeHistory{}, "history", "usage", "--since", "all"); got != "No usage recorded.\n" {
		t.Errorf("empty usage history: %q", got)
	}
...
func TestHistoryCommandRejectsBadArguments(t *testing.T) {
	for _, args := range [][]string{
		...
		{"history", "usage", "--since", "-2d"}, {"history", "usage", "--since", "NaNd"}, {"history", "extra"},
	} {
...
func TestHistoryEndToEnd(t *testing.T) {
	...
	usage, _ := run("history", "usage", "--csv")
	if lines := strings.Split(strings.TrimSpace(usage), "\n"); len(lines) != 3 || !strings.Contains(lines[1], ",codex,primary,,40,60,,2") || !strings.Contains(lines[2], ",codex,primary,,0,100,,1") {
		t.Fatalf("usage:\n%s", usage)
	}
	resets, _ := run("history", "--csv")
	if lines := strings.Split(strings.TrimSpace(resets), "\n"); len(lines) != 3 || !strings.Contains(lines[1], ",manual,requested,,end-to-end,") || !strings.Contains(lines[2], ",manual,outcome,reset,end-to-end,3001f754dd60,") {
		t.Fatalf("resets:\n%s", resets)
	}
}
