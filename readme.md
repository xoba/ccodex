...
retries do not start new redemptions. Only one `watch --auto-reset` can run at a
time; a second exits with an error, while read-only watches are unaffected.
Before sending a request, watch reads your quota again and spends nothing unless
it is still low, which also covers a reset you redeemed elsewhere a moment
earlier. Each automatic request is written to the history before it is sent; if
it cannot be recorded, it is not sent.
